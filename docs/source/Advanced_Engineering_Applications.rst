...
.. math::
   Den = \cfrac{1 + 4y + 4y^2 - 4y^3 + y^4}{(1 - y)^4} - 2By + CDy^{(D - 1)}\\
   c_rT_r = \cfrac{A Tr}/{yDen};
...
.. tabs::
   ...
   .. tab:: CCL-Math
      ...
      .. code-block:: C#
         ...
         // set up ressure and temperature mesh
         double[] Pr = Enumerable.Range(0, 501).Select(i => i * 0.05+0.2).ToArray();
         double[] Tr = [1.05,    1.10,   1.20,   1.30,   1.40];
         plt = new ChartHandle(); List<string> Tlabels = [];
         for (int j = 0; j < Tr.Length; j++)
         {
             plt.AddLogLog(Pr, Pr.Select(p => CrTrHY(p, Tr[j])).ToArray());
             Tlabels.Add("Tr = " + Tr[j]);
         }
         plt.Axis([0.1, 100, 0.01, 10]);
         plt.Legend = new() { labels = [..Tlabels], alignment = "lowerleft" };
         plt.SaveFig("Reduced-Compressibility-Hall-Yarborough.png");
